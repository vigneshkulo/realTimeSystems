...
# Taken From Online Source - Starts
	...
	for i in range(1, len(period)+1):
		...
		#       print "* Job %s: [%d to %d]" % (vName, startJ, deadlineJ)
			for k in range(0, fNum):
				...
				startF = k * f
				deadlineF = startF + f
				if(deadlineJ > hyperPeriod):
					residue = deadlineJ - hyperPeriod
					startR = 0
					deadlineR = residue
					if((startR <= startF) and (deadlineR >= deadlineF)):
					#	print "* %s: [%d - %d] : %d" % (vName, startJ, deadlineJ, hyperPeriod),
...
fExec = open('Execution Times2.txt', 'w')
for i in range(1, len(period)+1):
	vNum = hyperPeriod / period[i-1]
	for j in range(0, vNum):
		vName = "Task" + str(i) + "Job" + str(j+1)
	...
	#       print "* Job %s: [%d to %d]" % (vName, startJ, deadlineJ)
		for k in range(0, fNum):
			...
			deadlineF = startF + f
			if((startJ <= startF) and (deadlineJ >= deadlineF)):
				if(0 != flow[vList.index(vName)+1][len(vList) + fList.index(fName)+1]):
					fExec.write("%s executes in %s for %f\n" % (vName, fName, flow[vList.index(vName)+1][len(vList) + fList.index(fName)+1]))
		fExec.write("-" * 40)
		fExec.write("\n")
print "* Schedule and Execution Times are stored in 'Execution Times2.txt' file"
print "-------------------------------------"
print "* Part F: Frame Size for the Schedule:", f 
print "* Part G: Minor Cycle: %d, Major Cycle: %d" % (f, hyperPeriod/f) 
print "-------------------------------------"
